...
export async function enviar(req, res) {
  ...
  try {
    ...
    // Plantilla aprobada por Meta (dexon_aviso_admin): a diferencia de un texto libre, llega
    // aunque no le hayas escrito nada a Dexon en las últimas 24h (no depende de esa ventana).
    const detalle =
      tipo === "reprogramacion"
        ? `${fecha || "-"} a las ${horarios || "-"} · Reprogramación · Motivo: ${req.body.motivo || "-"}`
        : tipo === "confirmacion_presencial"
        ? `${fecha || "-"} a las ${horarios || "-"} · Confirmado presencial · Efectivo al llegar`
        : `${fecha || "-"} a las ${horarios || "-"} · Nueva reserva · ${monto || "-"} · ${metodo}`;
    const rAdmin = await sendTemplate(adminTel, {
      name: "dexon_aviso_admin", language: { code: "es" },
      components: [{ type: "body", parameters: [nombre, telefono, detalle].map(text => ({ type: "text", text: String(text ?? "-") })) }],
    });
