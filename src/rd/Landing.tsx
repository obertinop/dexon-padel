...
import {
  CLUB, gs, servicios, planesMkt as planes, testimonios, faqs, stats, instructoresMkt as instructores,
} from "@/data";
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {planes.map((p) => (
              <div key={p.nombre} className={cn(
                ...
                <p className="mt-1 text-sm text-muted-foreground">{p.horas}x por semana · horario fijo</p>
                ...
                <div className="mt-2 text-xs text-accent">
                  ≈ {gs(Math.round(p.precio / (p.horas * 4)))} por hora · ahorrás hasta 30%
                </div>
                <ul className="mt-5 space-y-2.5 text-sm">
                  {["Tu horario fijo reservado", "Reprogramación flexible", "Prioridad en torneos", p.horas >= 2 ? "1 clase mensual de regalo" : "Descuento en pro shop"].map((f) => (
      ...
      <section id="clases" className="mx-auto max-w-6xl px-5 py-20 md:py-28">
        <div className="grid gap-12 md:grid-cols-[0.9fr_1.1fr] md:items-center">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {instructores.map((p) => (
              <div key={p.nombre} className="rounded-lg border border-border bg-card p-6">
