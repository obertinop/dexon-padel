...
const Modal = ({ show, onClose, title, children, width=420 }) => {
  if (!show) return null;
  return (
    <div style={{position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"flex-start",justifyContent:"center",backgroundColor:"rgba(0,0,0,0.5)",padding:"24px 16px",overflowY:"auto"}}>
...
const Dialog = ({ show, title, msg, onOk, onCancel, okLabel="Confirmar", okV="danger" }) => {
  if (!show) return null;
  return (
    <div style={{position:"fixed",inset:0,zIndex:99999,display:"flex",alignItems:"center",justifyContent:"center",backgroundColor:"rgba(0,0,0,0.55)"}}>
...
const PortalCliente = () => {
  ...
  const buscarCliente = () => {
    ...
    const tel = form.telefono.trim().replace(/\D/g,"");
    const porNombre = clientes.filter(c=>c.nombre.toLowerCase()===nombre);
    const porTel = tel ? clientes.filter(c=>c.telefono.replace(/\D/g,"")===tel) : [];
    if (porNombre.length>0&&porTel.length>0&&porNombre[0].id===porTel[0].id) return {match:"total",cliente:porNombre[0]};
    if (porNombre.length>0) return {match:"parcial_nombre",cliente:porNombre[0]};
    if (porTel.length>0) return {match:"parcial_tel",cliente:porTel[0]};
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f7f6f4"}}>
      <div style={{width:360,background:"#fff",borderRadius:16,padding:"40px 32px",boxShadow:"0 8px 40px rgba(0,0,0,0.1)",border:"1px solid #eee"}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} style={{...inp,fontSize:14}} placeholder="tu@email.com"/>
        </div>
        <div style={{marginBottom:24}}>
          <label style={{fontSize:12,color:"#666",fontWeight:500,display:"block",marginBottom:5}}>Contraseña</label>
          <input type="password" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} style={{...inp,fontSize:14}} placeholder="••••••••"/>
        </div>
        <button onClick={doLogin} disabled={loading} style={{width:"100%",padding:"12px",background:C.coral,color:"#fff",border:"none",borderRadius:10,fontSize:15,fontWeight:500,cursor:"pointer",fontFamily:"var(--font-sans)"}}>
...
export default function App() {
  // Detectar si es portal del cliente
    ...
    fetch("https://api.open-meteo.com/v1/forecast?latitude=-25.65&longitude=-54.77&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_probability_max&timezone=America%2FAsuncion&forecast_days=5")
      ...
      .then(d => {
        const dias = d.daily.time.map((fecha,i) => ({
          fecha, max: Math.round(d.daily.temperature_2m_max[i]),
          min: Math.round(d.daily.temperature_2m_min[i]),
          lluvia: d.daily.precipitation_probability_max[i],
  ...
  const confirmarTurno = async t => {
    ...
    try {
      ...
      if (saldo>0) await db.post("caja",{descripcion:`${t.tipo==="clase"?"Clase":"Reserva"} - ${cById(t.cliente_id)?.nombre||"?"}`,tipo:"ingreso",categoria:t.tipo==="clase"?"clase":"reserva",monto:saldo,fecha:t.fecha,turno_id:t.id},tk);
  ...
  const noShow = async t => {
    setSaving(true);
    try { await db.patch("turnos",t.id,{estado:"no_show"},tk); await load(); setDlg(null); closeM(); }
    catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const guardarCliente = async () => {
    ...
    setSaving(true);
    try {
      const payload = {nombre:form.nombre.trim(),telefono:form.telefono||"",nivel:form.nivel||"intermedio",notas:form.notas||""};
      if (form.id) await db.patch("clientes",form.id,payload,tk);
      else await db.post("clientes",payload,tk);
      await load(); closeM();
    } catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const eliminarCliente = async id => {
    setSaving(true);
    try { await db.del("clientes",id,tk); await load(); setDlg(null); closeM(); }
    catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const cancelarAbono = async id => {
    setSaving(true);
    try { await db.patch("abonos",id,{estado:"cancelado"},tk); await load(); setDlg(null); }
    catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const guardarPlan = async () => {
    ...
    setSaving(true);
    try {
      if (form.id) await db.patch("planes",form.id,{nombre:form.nombre,horas_semana:Number(form.horas_semana),precio:Number(form.precio)},tk);
      else await db.post("planes",{nombre:form.nombre,horas_semana:Number(form.horas_semana),precio:Number(form.precio)},tk);
      await load(); closeM();
    } catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const guardarInstructor = async () => {
    ...
    setSaving(true);
    try {
      await db.post("instructores",{nombre:form.nombre.trim(),telefono:form.telefono||"",tarifa_clase:Number(form.tarifa_clase||0)},tk);
      await load(); closeM();
    } catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const guardarMovCaja = async () => {
    ...
    setSaving(true);
    try {
      await db.post("caja",{descripcion:form.descripcion,tipo:form.tipo||"egreso",categoria:form.categoria||"gasto",monto:Number(form.monto),fecha:form.fecha||hoy()},tk);
      await load(); closeM();
    } catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const eliminarMovCaja = async id => {
    setSaving(true);
    try { await db.del("caja",id,tk); await load(); setDlg(null); }
    catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const guardarStock = async () => {
    ...
    setSaving(true);
    try {
      const payload = {nombre:form.nombre,categoria:form.categoria||"general",cantidad:Number(form.cantidad),minimo:Number(form.minimo||0),precio_venta:Number(form.precio_venta||0),precio_costo:Number(form.precio_costo||0)};
      if (form.id) await db.patch("stock",form.id,payload,tk);
      else await db.post("stock",payload,tk);
      await load(); closeM();
    } catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const moverStock = async () => {
    ...
    try {
      ...
      await db.post("stock_movimientos",{stock_id:item.id,tipo:form.tipo_mov,cantidad:Number(form.cantidad_mov),motivo:form.motivo||"",fecha:hoy()},tk);
      if (form.tipo_mov==="salida"&&item.precio_venta>0)
        await db.post("caja",{descripcion:`Venta - ${item.nombre} x${form.cantidad_mov}`,tipo:"ingreso",categoria:"stock",monto:item.precio_venta*Number(form.cantidad_mov),fecha:hoy()},tk);
      if (form.tipo_mov==="entrada"&&item.precio_costo>0)
        await db.post("caja",{descripcion:`Compra - ${item.nombre} x${form.cantidad_mov}`,tipo:"egreso",categoria:"stock",monto:item.precio_costo*Number(form.cantidad_mov),fecha:hoy()},tk);
      await load(); closeM();
    } catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const guardarConfig = async () => {
    setSaving(true);
    try {
      await db.patch("config",cfg.id,{nombre_club:form.nombre_club,hora_inicio:Number(form.hora_inicio),hora_fin:Number(form.hora_fin),tarifa_base:Number(form.tarifa_base),tarifa_pico:Number(form.tarifa_pico),hora_pico_inicio:Number(form.hora_pico_inicio),hora_pico_fin:Number(form.hora_pico_fin)},tk);
      await load(); closeM();
    } catch(e) { alert(e.message); }
    setSaving(false);
  };
  ...
  const whatsapp = (c, t, msg) => {
    ...
    const m = msg||`Hola ${c.nombre}, tu turno en ${cfg.nombre_club} está confirmado para el ${t.fecha} a las ${t.hora}:00hs.`;
    ...
    return <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          <Btn v="ghost" sm onClick={doLogout}>Salir</Btn>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:16}}>
        {[
          {l:"Ingresos hoy",v:gs(ingH)},
          {l:"Ingresos mes",v:gs(ingM)},
          {l:"Balance mes",v:gs(ingM-egrM),c:ingM-egrM>=0?C.ok:C.danger},
          {l:"Turnos hoy",v:tHoy.length,sub:pendCobro.length>0?`${pendCobro.length} pendientes de cobro`:null},
        ].map((m,i)=><div key={i} style={metric}>
          <div style={{fontSize:12,color:"var(--color-text-secondary)",marginBottom:6}}>{m.l}</div>
          <div style={{fontSize:21,fontWeight:500,color:m.c||"var(--color-text-primary)"}}>{m.v}</div>
          {m.sub&&<div style={{fontSize:11,color:C.warn,marginTop:3}}>{m.sub}</div>}
        </div>)}
      </div>
      ...
      {clima&&<div style={{...card,marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto"}}>
          {clima.map((d,i)=>{
            ...
            const esHoy = d.fecha===h;
            const lluvia = d.lluvia>=60;
            return <div key={i} style={{flex:1,minWidth:70,textAlign:"center",padding:"10px 8px",borderRadius:10,background:esHoy?"#FAECE7":lluvia?"#E6F1FB":"var(--color-background-secondary)",border:esHoy?`1px solid ${C.coral}`:"none"}}>
              <div style={{fontSize:11,color:"var(--color-text-secondary)",marginBottom:4}}>{esHoy?"Hoy":DIAS[fecha.getDay()]}</div>
        ...
        {tHoy.length===0?<Empty t="Sin turnos para hoy"/>:
          <div style={{display:"grid",gap:8}}>
            {tHoy.map(t=>{
              ...
              return <div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",borderRadius:10,background:"var(--color-background-secondary)",border:"0.5px solid var(--color-border-tertiary)",cursor:"pointer"}} onClick={()=>openM("verTurno",{...t,cliente:c,instructor:ins})}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:"var(--color-text-secondary)",marginTop:2,display:"flex",gap:6,flexWrap:"wrap"}}>
                    ...
                    {ins&&<span>· {ins.nombre}</span>}
                    {t.sena>0&&<span style={{color:C.ok}}>· Seña: {gs(t.sena)}</span>}
                  </div>
                </div>
      ...
      <div style={{overflowX:"auto"}}>
        <div style={{display:"grid",gridTemplateColumns:`52px repeat(7,1fr)`,gap:1,background:"var(--color-border-tertiary)",borderRadius:10,overflow:"hidden",minWidth:600}}>
          ...
          {horas.map(h=><>
            ...
            {dias.map((d,di)=>{
              const fs = fmtD(d);
              const t = all.find(t=>t.fecha===fs&&t.hora===h&&t.estado!=="cancelado");
              const c = t?cById(t.cliente_id):null;
              const isPico = h>=cfg.hora_pico_inicio&&h<cfg.hora_pico_fin;
              return <div key={`${h}-${di}`}
                onClick={()=>t?openM("verTurno",{...t,cliente:c,instructor:iById(t.instructor_id)}):openM("turno",{fecha:fs,hora:h,tipo:"ocasional"})}
                style={{background:t?(t.tipo==="abono"?C.purpleL:t.tipo==="clase"?C.infoL:C.coralL):(isPico?"rgba(216,90,48,0.04)":"var(--color-background-primary)"),display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",minHeight:40}}>
    ...
    return <div>
      ...
      <div style={{display:"grid",gap:8}}>
        {lista.map(c=>{
          ...
          return <div key={c.id} style={{...card,display:"flex",alignItems:"center",gap:14,padding:"12px 16px",cursor:"pointer",transition:"background .1s"}}
            onClick={()=>openM("cliente",{...c})}>
  ...
  const Abonados = () => {
    ...
    return <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <Btn v="ghost" onClick={()=>openM("plan",{})}>Gestionar planes</Btn>
          <Btn v="primary" onClick={()=>openM("abono",{fecha_inicio:hoy(),slots:[]})}>+ Nuevo abono</Btn>
        </div>
      </div>
      ...
      <div style={{display:"grid",gap:8}}>
        {[...venc,...vig].map(ab=>{
          const c = cById(ab.cliente_id); const pl = pById(ab.plan_id); const v = ab.fecha_vencimiento<h;
          return <div key={ab.id} style={{...card,padding:"14px 16px"}}>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:12,color:"var(--color-text-tertiary)",marginTop:2}}>{diasAbono(ab)}</div>
              </div>
    ...
    return <div>
      ...
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",borderRadius:10,overflow:"hidden"}}>
          <thead><tr>
            {["Fecha","Descripción","Categoría","Monto",""].map((h,i)=><th key={i} style={{textAlign:i>=3?"right":"left",padding:"10px 14px",fontSize:12,fontWeight:500,color:"var(--color-text-secondary)",borderBottom:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)"}}>{h}</th>)}
          </tr></thead>
          <tbody>
            {caja.map(m=><tr key={m.id}>
              ...
              <td style={{padding:"10px 14px",fontSize:13,borderBottom:"0.5px solid var(--color-border-tertiary)"}}><Badge type={m.tipo==="ingreso"?"ok":"danger"}>{m.categoria||m.tipo}</Badge></td>
              ...
              <td style={{padding:"10px 14px",fontSize:13,borderBottom:"0.5px solid var(--color-border-tertiary)",textAlign:"right"}}>
                <Btn sm v="danger" onClick={()=>setDlg({type:"eliminarMov",id:m.id,desc:m.descripcion})}>×</Btn>
              </td>
            </tr>)}
          </tbody>
        </table>
  ...
  const Stock = () => {
    ...
    return <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <Btn v="ghost" onClick={()=>openM("moverStock",{tipo_mov:"salida"})}>Registrar movimiento</Btn>
          <Btn v="primary" onClick={()=>openM("stockItem",{categoria:"pelotas",cantidad:"0",minimo:"0"})}>+ Producto</Btn>
        </div>
      </div>
      ...
      {cats.map(cat=><div key={cat} style={{marginBottom:16}}>
        ...
        <div style={{display:"grid",gap:8}}>
          {stock.filter(s=>s.categoria===cat).map(s=>{
            ...
            return <div key={s.id} style={{...card,display:"flex",alignItems:"center",gap:14,padding:"12px 16px",cursor:"pointer"}} onClick={()=>openM("stockItem",{...s})}>
              <div style={{flex:1}}>
                <div style={{fontWeight:500,fontSize:14}}>{s.nombre}</div>
                {(s.precio_venta>0||s.precio_costo>0)&&<div style={{fontSize:12,color:"var(--color-text-secondary)",marginTop:2}}>Venta: {gs(s.precio_venta)} · Costo: {gs(s.precio_costo)}</div>}
              </div>
              <div style={{display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
                <div style={{textAlign:"center"}}>
                  <div style={{fontSize:24,fontWeight:500,color:b?C.danger:"var(--color-text-primary)"}}>{s.cantidad}</div>
                  <div style={{fontSize:10,color:"var(--color-text-tertiary)"}}>unidades</div>
                </div>
                {b&&<Badge type="danger">Bajo</Badge>}
              </div>
            </div>;
          })}
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <div style={card}>
          ...
          {topC.map((c,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:"0.5px solid var(--color-border-tertiary)"}}>
            ...
            <Badge type="info">{c.n} turnos</Badge>
          </div>)}
        </div>
      </div>
    </div>;
  ...
  const Config = () => <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:8}}>
        <Btn v="ghost" onClick={()=>openM("instructor",{})}>+ Instructor</Btn>
        <Btn v="primary" onClick={()=>openM("config",{...cfg})}>Editar</Btn>
      </div>
    </div>
    ...
    {instructores.length>0&&<div style={{...card,marginBottom:12}}>
      <div style={{fontWeight:500,marginBottom:12,fontSize:14}}>Instructores</div>
      {instructores.map(i=><div key={i.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"0.5px solid var(--color-border-tertiary)",fontSize:13}}>
        <span style={{fontWeight:500}}>{i.nombre}</span><span style={{color:"var(--color-text-secondary)"}}>{gs(i.tarifa_clase)}/clase</span>
      </div>)}
    </div>}
    <div style={card}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{fontWeight:500,fontSize:14}}>Planes de abono</div>
        <Btn sm v="ghost" onClick={()=>openM("plan",{})}>+ Plan</Btn>
      </div>
      {planes.map(p=><div key={p.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:"0.5px solid var(--color-border-tertiary)",fontSize:13}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <Btn sm v="ghost" onClick={()=>openM("plan",{...p})}>Editar</Btn>
        </div>
      </div>)}
    </div>
  </div>;
  ...
  return (
    <div style={{fontFamily:"var(--font-sans)",maxWidth:940,margin:"0 auto",padding:"0 2px"}}>
      ...
      <Modal show={modal==="turno"} onClose={closeM} title="Nueva reserva">
        ...
        {form.tipo==="ocasional"&&<>
          <div style={{background:"var(--color-background-secondary)",borderRadius:8,padding:"10px 12px",fontSize:13,marginBottom:14}}>
            Precio: <strong>{gs(precioTurno(Number(form.hora||cfg.hora_inicio)))}</strong>
            {Number(form.hora)>=cfg.hora_pico_inicio&&Number(form.hora)<cfg.hora_pico_fin&&<span style={{color:C.coral}}> (pico)</span>}
          </div>
      ...
      <Modal show={modal==="verTurno"} onClose={closeM} title="Detalle del turno">
        {form.cliente&&<>
          ...
          <div style={{...metric,marginBottom:14,display:"grid",gap:8}}>
            {[["Fecha/Hora",`${form.fecha} · ${form.hora}:00hs`],["Tipo",tipoBadge(form.tipo)],["Estado",estadoBadge(form.estado)],["Precio",gs(form.precio)],form.sena>0&&["Seña cobrada",<span style={{color:C.ok}}>{gs(form.sena)}</span>],form.sena>0&&["Saldo",<strong>{gs(form.precio-(form.sena||0))}</strong>],form.instructor&&["Instructor",form.instructor.nombre],form.notas&&["Notas",form.notas]].filter(Boolean).map(([k,v],i)=>
              <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:13}}><span style={{color:"var(--color-text-secondary)"}}>{k}</span><span>{v}</span></div>
            )}
          </div>
          <Div/>
          ...
          {form.estado==="confirmado"&&<Btn v="success" onClick={()=>whatsapp(form.cliente,form)}>Enviar confirmación WhatsApp</Btn>}
        </>}
      </Modal>
      ...
      <Modal show={modal==="cliente"} onClose={closeM} title={form.id?"Editar cliente":"Nuevo cliente"}>
        ...
        <div style={{display:"flex",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",gap:8,marginLeft:"auto"}}>
            <Btn onClick={closeM}>Cancelar</Btn>
            <Btn v="primary" onClick={guardarCliente} disabled={saving}>{saving?"Guardando...":form.id?"Guardar cambios":"Agregar"}</Btn>
          </div>
        </div>
      </Modal>
      ...
      <Modal show={modal==="abono"} onClose={closeM} title="Registrar abono">
        ...
        <div style={{marginBottom:14}}>
          ...
          {(form.slots||[]).map((slot,i)=>(
            <div key={i} style={{display:"flex",gap:8,alignItems:"center",marginBottom:8}}>
              ...
              <button type="button" onClick={()=>setForm(f=>({...f,slots:f.slots.filter((_,j)=>j!==i)}))} style={{border:"none",background:C.dangerL,color:C.danger,borderRadius:6,padding:"6px 10px",cursor:"pointer",fontFamily:"var(--font-sans)"}}>×</button>
            </div>
          ))}
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn onClick={closeM}>Cancelar</Btn>
          <Btn v="primary" onClick={guardarAbono} disabled={saving}>{saving?"Guardando...":"Registrar abono"}</Btn>
        </div>
      </Modal>
      ...
      <Modal show={modal==="plan"} onClose={closeM} title={form.id?"Editar plan":"Nuevo plan"}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn onClick={closeM}>Cancelar</Btn>
          <Btn v="primary" onClick={guardarPlan} disabled={saving}>{saving?"Guardando...":"Guardar plan"}</Btn>
        </div>
      </Modal>
      ...
      <Modal show={modal==="instructor"} onClose={closeM} title="Nuevo instructor">
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn onClick={closeM}>Cancelar</Btn>
          <Btn v="primary" onClick={guardarInstructor} disabled={saving}>{saving?"Guardando...":"Guardar"}</Btn>
        </div>
      </Modal>
      ...
      <Modal show={modal==="movCaja"} onClose={closeM} title="Registrar movimiento">
        ...
        <Sel label="Categoría" value={form.categoria||"gasto"} onChange={sf("categoria")}>
          <option value="gasto">Gasto operativo</option><option value="stock">Stock / compra</option><option value="otro">Otro</option>
        </Sel>
        <Inp label="Fecha" type="date" value={form.fecha||""} onChange={sf("fecha")}/>
        <Div/>
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn onClick={closeM}>Cancelar</Btn>
          <Btn v="primary" onClick={guardarMovCaja} disabled={saving}>{saving?"Guardando...":"Guardar"}</Btn>
        </div>
      </Modal>
      ...
      <Modal show={modal==="stockItem"} onClose={closeM} title={form.id?"Editar producto":"Nuevo producto"}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn onClick={closeM}>Cancelar</Btn>
          <Btn v="primary" onClick={guardarStock} disabled={saving}>{saving?"Guardando...":"Guardar"}</Btn>
        </div>
      </Modal>
      ...
      <Modal show={modal==="moverStock"} onClose={closeM} title="Movimiento de stock">
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn onClick={closeM}>Cancelar</Btn>
          <Btn v="primary" onClick={moverStock} disabled={saving}>{saving?"Guardando...":"Confirmar"}</Btn>
        </div>
      </Modal>
      ...
      <Modal show={modal==="config"} onClose={closeM} title="Configuración">
        ...
        <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
          <Btn onClick={closeM}>Cancelar</Btn>
          <Btn v="primary" onClick={guardarConfig} disabled={saving}>{saving?"Guardando...":"Guardar"}</Btn>
        </div>
      </Modal>
      ...
      <Dialog show={dlg?.type==="eliminarMov"} title="Eliminar movimiento" msg={`¿Eliminar "${dlg?.desc}" de caja? Esta acción no se puede deshacer.`} onOk={()=>eliminarMovCaja(dlg.id)} onCancel={()=>setDlg(null)} okLabel="Eliminar" okV="danger"/>

      {/* WhatsApp post-reserva */}
